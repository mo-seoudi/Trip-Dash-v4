...
export async function getAuthorizedOperationalContext({ identity, schoolId, requiredPermission = null }) {
  ...
  if (requiredPermission && !workspace.permissions.includes(requiredPermission)) throw new OperationalAuthorizationError("Required permission is not available in this school workspace", "OPERATIONAL_PERMISSION_DENIED");
  const prisma = await getOperationalPrismaForOrganization(workspace.schoolId);
  return { user: access.user, workspace, prisma };
}
