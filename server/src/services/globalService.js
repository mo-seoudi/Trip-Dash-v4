...
export const setActiveOrg = async (org_id) =>
  api.post("/session/set-org", { org_id }); // 204 on success

// ---- tenants ----
export const listTenants = async () =>
  (await api.get("/global/tenants")).data;
export const createTenant = async (payload) =>
  (await api.post("/global/tenants", payload)).data; // {name, slug}

// ---- organizations ----
export const listOrgs = async (tenant_id) =>
  (await api.get("/global/orgs", { params: { tenant_id } })).data;
export const createOrg = async (payload) =>
  (await api.post("/global/orgs", payload)).data; // {tenant_id, name, type, code?, parent_org_id?}
...
export const createPartnership = async (payload) =>
  (await api.post("/global/partnerships", payload)).data; // {tenant_id, school_org_id, bus_company_org_id}
