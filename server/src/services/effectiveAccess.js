...
function addAccess(map, org, detail) {
  ...
  if (!existing.access.some((item) => JSON.stringify(item) === signature)) existing.access.push(detail);
}
...
async function findGlobalUser(prisma, legacyUser) {
  return (
...
export async function resolveEffectiveAccessWithPrisma(prisma, legacyUser) {
  if (!prisma) throw new TypeError("legacy global Prisma client is required");
  const gUser = await findGlobalUser(prisma, legacyUser);

  if (!gUser || !gUser.isActive) {
    const role = canonicalRoleKey(legacyUser.role);
    return {
      source: "legacy-unscoped",
      user: { id: String(legacyUser.id), email: legacyUser.email, displayName: legacyUser.name },
      roles: role ? [role] : [],
      permissions: role ? permissionsForRoles([role]) : [],
      organizations: [], workspaces: [], portfolio: { enabled: false, schoolCount: 0 },
    };
  }

  const memberships = await prisma.userOrgMembership.findMany({
    where: { userId: gUser.id }, include: { org: true },
  ...
  const addSchoolRole = (schoolId, role) => {
    ...
    set.add(role); schoolRoleMap.set(schoolId, set);
  };
  ...
  for (const membership of activeMemberships) {
    ...
    if (orgType === CANONICAL_ORGANIZATION_TYPES.SCHOOL) addSchoolRole(org.id, role);

    if (orgType === CANONICAL_ORGANIZATION_TYPES.SCHOOL_GROUP) {
      const schools = await prisma.organization.findMany({
        where: { tenantId: org.tenantId, parentOrgId: org.id, type: "school" }, orderBy: { name: "asc" },
      });
      for (const school of schools) {
        addAccess(accessMap, school, { kind: "relationship", role, viaOrganizationId: org.id, viaRelationship: "BELONGS_TO_GROUP" });
    ...
    if (orgType === CANONICAL_ORGANIZATION_TYPES.BUS_OPERATOR) {
      const links = await prisma.partnership.findMany({
        where: { busCompanyOrgId: org.id, status: ACTIVE_RELATIONSHIP_STATUS }, include: { school: true },
      });
      for (const link of links) {
        if (tenantId && link.tenantId !== tenantId) continue;
        addAccess(accessMap, link.school, { kind: "relationship", role: ROLE_KEYS.BUS_OPERATOR, viaOrganizationId: org.id, viaRelationship: "TRANSPORT_PROVIDER" });
        addSchoolRole(link.schoolOrgId, ROLE_KEYS.BUS_OPERATOR);
      }
    }
  }

  const scopes = await prisma.userOrgScope.findMany({
    where: { userId: gUser.id }, select: { orgId: true, role: true, schoolOrgId: true },
  });
  const scopedByOrgRole = new Map();
  for (const scope of scopes) {
    const key = `${scope.orgId}:${canonicalRoleKey(scope.role)}`;
    const set = scopedByOrgRole.get(key) || new Set(); set.add(scope.schoolOrgId); scopedByOrgRole.set(key, set);
  }
  ...
  for (const [schoolId] of [...schoolRoleMap.entries()]) {
    ...
    if (!permittedRoles.size) { accessMap.delete(schoolId); schoolRoleMap.delete(schoolId); }
    else schoolRoleMap.set(schoolId, permittedRoles);
  }
  ...
  const workspaces = organizations.filter((org) => org.type === CANONICAL_ORGANIZATION_TYPES.SCHOOL).map((school) => {
    const workspaceRoles = [...(schoolRoleMap.get(school.id) || [])].filter(Boolean).sort();
    return { schoolId: school.id, displayName: school.displayName, fullName: school.fullName, abbreviation: school.abbreviation,
      roles: workspaceRoles, permissions: workspacePermissions(workspaceRoles), access: school.access };
  });

  return {
    source: "legacy-global-bridge",
    user: { id: String(legacyUser.id), appUserId: gUser.id, email: legacyUser.email, displayName: gUser.fullName || legacyUser.name },
    tenantId, roles: roles.sort(), permissions: permissionsForRoles(roles), organizations, workspaces,
    portfolio: { enabled: workspaces.length > 0, schoolCount: workspaces.length },
  };
}

// Existing runtime API remains unchanged. Routes continue using the legacy
// singleton until a controlled canonical cutover is explicitly enabled.
export async function resolveEffectiveAccess(legacyUser) {
  return resolveEffectiveAccessWithPrisma(prismaGlobal, legacyUser);
}
