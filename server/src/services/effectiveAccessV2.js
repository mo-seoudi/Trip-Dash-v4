...
function inheritedGroupRole(role) {
  if (role === ROLE_KEYS.FINANCE) return ROLE_KEYS.FINANCE;
  if (role === ROLE_KEYS.TENANT_ADMIN) return ROLE_KEYS.TENANT_ADMIN;
  return ROLE_KEYS.GROUP_STAFF;
}
...
async function activeChildrenOfGroups(prisma, groupIds, now) {
  ...
  for (const groupId of groupIds) {
    const rows = await prisma.organizationRelationship.findMany({
      where: { toOrganizationId: groupId, type: "BELONGS_TO_GROUP" },
...
export async function resolveEffectiveAccessV2(prisma, identity, { now = new Date() } = {}) {
  ...
  if (!selectors.length) return { source: "canonical-v2", user: null, roles: [], permissions: [], organizations: [], workspaces: [], portfolio: { enabled: false, schoolCount: 0 } };
  ...
  if (!user || !activeStatus(user.status)) return {
    source: "canonical-v2",
    user: user ? { id: String(identity?.id || ""), appUserId: user.id, email: user.email, displayName: user.displayName } : null,
    roles: [], permissions: [], organizations: [], workspaces: [], portfolio: { enabled: false, schoolCount: 0 },
  };
  ...
  const organizationAssignments = assignments.filter((item) => item.scopeType === "ORGANIZATION" && item.organizationId);

  // A role may target the user's own organization or, for a group employee,
  // an explicitly selected child school. This enables restricted group finance
  // and admin portfolios without making tenant coverage an access mechanism.
  const validAssignments = organizationAssignments.filter((assignment) =>
    memberOrgIds.has(assignment.organizationId) || childSchoolIds.has(assignment.organizationId));
  ...
  for (const assignment of validAssignments) {
    const detail = { kind: "direct", role: roleKey(assignment), viaOrganizationId: assignment.organizationId };
    addOrganization(organizationMap, assignment.organization, detail);
    if (assignment.organization?.type === "SCHOOL") addWorkspace(workspaceMap, assignment.organization, roleKey(assignment), detail);
  }

  for (const assignment of validAssignments.filter((item) => item.organization?.type === "SCHOOL_GROUP")) {
    const rows = await prisma.organizationRelationship.findMany({ where: { toOrganizationId: assignment.organizationId, type: "BELONGS_TO_GROUP" }, include: { fromOrganization: true } });
    for (const relationship of rows.filter((item) => activeRelationship(item, now))) {
      const role = inheritedGroupRole(roleKey(assignment));
      const detail = { kind: "relationship", role, viaOrganizationId: assignment.organizationId, viaRelationship: "BELONGS_TO_GROUP" };
      addWorkspace(workspaceMap, relationship.fromOrganization, role, detail);
      addOrganization(organizationMap, relationship.fromOrganization, detail);
    }
  }
  for (const assignment of validAssignments.filter((item) => item.organization?.type === "BUS_OPERATOR")) {
    const rows = await prisma.organizationRelationship.findMany({ where: { toOrganizationId: assignment.organizationId, type: "TRANSPORT_PROVIDER" }, include: { fromOrganization: true } });
    for (const relationship of rows.filter((item) => activeRelationship(item, now))) {
      const role = inheritedOperatorRole(roleKey(assignment));
      const detail = { kind: "relationship", role, viaOrganizationId: assignment.organizationId, viaRelationship: "TRANSPORT_PROVIDER" };
      addWorkspace(workspaceMap, relationship.fromOrganization, role, detail);
      addOrganization(organizationMap, relationship.fromOrganization, detail);
    }
  }
  for (const assignment of validAssignments.filter((item) => item.organization?.type === "SERVICE_PARTNER")) {
    const rows = await prisma.organizationRelationship.findMany({ where: { toOrganizationId: assignment.organizationId, type: "TRIP_MANAGER" }, include: { fromOrganization: true } });
    for (const relationship of rows.filter((item) => activeRelationship(item, now))) {
      const role = inheritedPartnerRole(roleKey(assignment));
      const detail = { kind: "relationship", role, viaOrganizationId: assignment.organizationId, viaRelationship: "TRIP_MANAGER" };
      addWorkspace(workspaceMap, relationship.fromOrganization, role, detail);
      addOrganization(organizationMap, relationship.fromOrganization, detail);
    }
  }
  ...
  const workspaces = [...workspaceMap.values()]
    ...
    .map(({ organization, roles, access }) => {
      const resolvedRoles = [...roles].sort();
      return { schoolId: organization.id, displayName: organization.displayName, fullName: organization.fullName, abbreviation: organization.abbreviation, roles: resolvedRoles, permissions: permissionsForRoles(resolvedRoles), access };
    });
  ...
  return {
    ...
    permissions: permissionsForRoles([...operationalRoles, ...platformAssignments.map(roleKey)]),
    organizations,
    workspaces,
    platformAccess: {
      roles: [...new Set(platformAssignments.map(roleKey).filter(Boolean))].sort(),
      tenantScopes: tenantAssignments.map((item) => ({ tenantId: item.tenantId, role: roleKey(item) })),
    },
