...
app.get("/api/me", requireAuth, async (req, res, next) => {
  try {
    ...
    const roles = gUser ? await prismaGlobal.userOrgMembership.findMany({
      where: { userId: gUser.id }, include: { org: { select: { id: true, name: true, type: true } } }, orderBy: { orgId: "asc" },
    }) : [];
    return res.json({
      user: { id: appUser.id, email: appUser.email, name: appUser.name, role: appUser.role },
      orgs: roles.map((r) => ({
        org_id: r.orgId, name: r.org?.name || r.orgId,
...
app.post("/api/session/set-org", requireAuth, async (req, res, next) => {
  try {
    ...
    if (!org_id || typeof org_id !== "string" || org_id.length > 100) return res.status(400).json({ message: "valid org_id required" });
    ...
    const membership = await prismaGlobal.userOrgMembership.findFirst({
      where: { userId: gUser.id, orgId: org_id }, select: { userId: true, orgId: true, status: true },
    });
    ...
    res.cookie("td_active_org", org_id, { httpOnly: true, sameSite: isProd ? "none" : "lax", secure: isProd, path: "/" });
