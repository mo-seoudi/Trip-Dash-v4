...
export function createTripService({ prisma, workspace, user }) {
  ...
  async function hydrated(id) {
    return prisma.trip.findUnique({ where: { id }, include: { busAssignments: true, passengers: true, quotations: true, approvalRequests: true } });
  }
  ...
  async function transition(id, fromStatus, toStatus) {
    ...
    const changed = await prisma.trip.updateMany({
      where: { id: current.id, owningSchoolOrganizationId: workspace.schoolId, status: current.status, OR: [{ cancelRequest: false }, { cancelRequest: null }] },
  ...
  return {
    ...
    async get(id) {
      const trip = await prisma.trip.findFirst({ where: schoolWhere(workspace, { id: tripId(id) }), include: { busAssignments: true, passengers: true, quotations: true, approvalRequests: true } });
      if (!trip) throw new TripNotFoundError();
      return trip;
    },
    ...
    async cancel(id) {
      ...
      return prisma.trip.update({ where:{id:current.id}, data:{status:STATUS.CANCELED,cancelRequest:false}, include:{busAssignments:true} });
    },
    ...
    async requestCancellation(id) {
      ...
      const changed = await prisma.trip.updateMany({ where:{id:current.id,owningSchoolOrganizationId:workspace.schoolId,status:current.status,OR:[{cancelRequest:false},{cancelRequest:null}]}, data:{cancelRequest:true} });
    ...
    async resolveCancellation(id, approve) {
      ...
      const changed = await prisma.trip.updateMany({ where:{id:current.id,owningSchoolOrganizationId:workspace.schoolId,status:current.status,cancelRequest:true}, data });
