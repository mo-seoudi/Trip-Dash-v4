...
function tenantView(tenant) { return { id: tenant.id, name: tenant.name, slug: tenant.slug, status: tenant.status, subscription_mode: tenant.subscriptionMode, plan_key: tenant.planKey }; }
...
// Tenants are administration/billing scopes, not operational workspaces. A
// platform super admin can administer every tenant; tenant admins see only the
// tenants explicitly assigned to them.
router.get("/tenants", async (req, res, next) => {
  try {
    const user = await appUserForRequest(req);
    if (!user) return res.status(403).json({ message: "Canonical user identity not found" });
    const assignments = await prismaControl.roleAssignment.findMany({
      where: { userId: user.id, isActive: true, OR: [{ scopeType: "PLATFORM", role: { key: "super_admin" } }, { scopeType: "TENANT", role: { key: { in: ["tenant_admin", "super_admin"] } } }] },
      include: { role: true },
    });
    const platformAdmin = assignments.some((assignment) => assignment.scopeType === "PLATFORM" && assignment.role?.key === "super_admin");
    const tenantIds = [...new Set(assignments.filter((assignment) => assignment.scopeType === "TENANT" && assignment.tenantId).map((assignment) => assignment.tenantId))];
    const tenants = await prismaControl.tenant.findMany({
      where: platformAdmin ? {} : { id: { in: tenantIds } },
      orderBy: { name: "asc" },
    });
    return res.json(tenants.map(tenantView));
...
router.post("/relationships", async (req, res, next) => { try { const type = String(req.body?.type || "").trim().toUpperCase(); if (!RELATIONSHIP_TYPES.has(type)) return res.status(400).json({ message: "Unsupported relationship type" }); const fromOrganizationId = text(req.body?.from_organization_id, "from_organization_id", { required: true, max: 100 }); const toOrganizationId = text(req.body?.to_organization_id, "to_organization_id", { required: true, max: 100 }); if (fromOrganizationId === toOrganizationId) return res.status(400).json({ message: "An organization cannot relate to itself" }); const notes = text(req.body?.notes, "notes", { max: 500 }); await Promise.all([assertOrganizationAdminAccess(req, fromOrganizationId), assertOrganizationAdminAccess(req, toOrganizationId)]); const created = await prismaControl.organizationRelationship.create({ data: { type, fromOrganizationId, toOrganizationId, status: "active", isPrimary: Boolean(req.body?.is_primary), notes } }); return res.status(201).json(created); } catch (e) { if (e?.code === "P2002") return res.status(409).json({ message: "This organization relationship already exists" }); return next(e); } });
router.patch("/relationships/:id", async (req, res, next) => { try { const id = text(req.params.id, "relationship id", { required: true, max: 100 }); const existing = await prismaControl.organizationRelationship.findUnique({ where: { id } }); if (!existing) return res.status(404).json({ message: "Relationship not found" }); await Promise.all([assertOrganizationAdminAccess(req, existing.fromOrganizationId), assertOrganizationAdminAccess(req, existing.toOrganizationId)]); const data = {}; if (req.body?.status !== undefined) data.status = text(req.body.status, "status", { required: true, max: 50 }); if (req.body?.is_primary !== undefined) data.isPrimary = Boolean(req.body.is_primary); if (req.body?.notes !== undefined) data.notes = text(req.body.notes, "notes", { max: 500 }); if (!Object.keys(data).length) return res.status(400).json({ message: "No supported fields to update" }); return res.json(await prismaControl.organizationRelationship.update({ where: { id }, data })); } catch (e) { return next(e); } });
router.delete("/relationships/:id", async (req, res, next) => { try { const id = text(req.params.id, "relationship id", { required: true, max: 100 }); const existing = await prismaControl.organizationRelationship.findUnique({ where: { id } }); if (!existing) return res.status(404).json({ message: "Relationship not found" }); await Promise.all([assertOrganizationAdminAccess(req, existing.fromOrganizationId), assertOrganizationAdminAccess(req, existing.toOrganizationId)]); await prismaControl.organizationRelationship.delete({ where: { id } }); return res.status(204).end(); } catch (e) { return next(e); } });
