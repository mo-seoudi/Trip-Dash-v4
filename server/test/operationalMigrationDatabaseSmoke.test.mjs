...
test("fixed-precision legacy prices survive plan, PostgreSQL write, and verification", { skip: !enabled }, async () => {
  assertDisposableUrl(process.env.OPERATIONAL_DATABASE_URL);
  const prisma = new PrismaOperational();
  try {
    await reset(prisma);
    const plan = buildOperationalMigrationPlan([{
      id: 504, price: "1234.567", buses: [{ busType: "White Bus", busSeats: 20, tripPrice: "450.505" }],
    }], () => ({ owningSchoolOrganizationId: "school-smoke" }));
    assert.deepEqual(plan.errors, []);
    assert.equal(plan.rows[0].trip.price, "1234.57");
    assert.equal(plan.rows[0].busAssignments[0].price, "450.51");
    ...
    const stored = await prisma.trip.findUnique({ where: { id: 504 }, include: { busAssignments: true } });
    assert.equal(stored.price.toFixed(2), "1234.57");
    assert.equal(stored.busAssignments[0].price.toFixed(2), "450.51");
  } finally {
...
test("invalid legacy trip money blocks PostgreSQL migration before writes", { skip: !enabled }, async () => {
  assertDisposableUrl(process.env.OPERATIONAL_DATABASE_URL);
  const prisma = new PrismaOperational();
  try {
    await reset(prisma);
    const plan = buildOperationalMigrationPlan([{ id: 505, price: "AED 1,000" }], () => ({ owningSchoolOrganizationId: "school-smoke" }));
    assert.equal(plan.errors[0].code, "OPERATIONAL_MIGRATION_INVALID_MONEY");
...
test("JSON buses plus historical SubTrips reconcile, write, and verify exactly in PostgreSQL", { skip: !enabled }, async () => {
  ...
  try {
    await reset(prisma);
    const legacyTrips = [{ id: 501, destination: "Expo City", status: "Confirmed", students: 42, staff: 3, buses: [
    ...
    const plan = buildOperationalMigrationPlan(legacyTrips, () => ({ owningSchoolOrganizationId: "school-smoke", tenantId: "tenant-smoke", subTrips }));
    assert.deepEqual(plan.errors, []);
  ...
  } finally { await reset(prisma).catch(() => {}); await prisma.$disconnect(); }
});
...
test("conflicting SubTrip source blocks PostgreSQL migration before writes", { skip: !enabled }, async () => {
  ...
  try {
    await reset(prisma);
    const plan = buildOperationalMigrationPlan([{ id: 503, buses: [{ busType: "White Bus", busSeats: 20, tripPrice: 400 }] }], () => ({ owningSchoolOrganizationId: "school-smoke", subTrips: [{ id: 7003, parentTripId: 503, busType: "White Bus", busSeats: 35, tripPrice: 400 }] }));
