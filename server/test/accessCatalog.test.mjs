...
test("legacy role aliases normalize to canonical role keys", () => {
  assert.equal(canonicalRoleKey("bus_company"), ROLE_KEYS.BUS_OPERATOR);
  assert.equal(canonicalRoleKey("operator"), ROLE_KEYS.BUS_OPERATOR);
  assert.equal(canonicalRoleKey("trip_manager"), ROLE_KEYS.SERVICE_PARTNER);
  assert.equal(canonicalRoleKey("admin"), ROLE_KEYS.TENANT_ADMIN);
  assert.equal(canonicalRoleKey("staff"), ROLE_KEYS.SCHOOL_STAFF);
});
...
test("tenant and super admins have access administration privileges", () => {
  ...
  assert.deepEqual(
    new Set(ROLE_PERMISSION_CATALOG[ROLE_KEYS.SUPER_ADMIN]),
    new Set(Object.values(PERMISSIONS)),
  );
});

test("permissionsForRoles canonicalizes aliases and de-duplicates permissions", () => {
  const permissions = permissionsForRoles(["bus_company", "bus_operator"]);
