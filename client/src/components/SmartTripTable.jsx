...
const SmartTripTable = ({ trips, dateSortOrder, setDateSortOrder, readOnly = false }) => {
  ...
  const { handleStatusChange, handleWorkflowAction, handleSoftDelete } = useTripActions(activeWorkspace, setTripData, setExpandedTripId);
  ...
  {expandedTripId===trip.id&&<tr className="bg-gray-50 border-b"><td colSpan={7} className="px-4 py-2"><div className="flex flex-col gap-2 sm:flex-row sm:gap-4"><ActionsCell trip={trip} onStatusChange={async(t,status)=>{try{const updated=await handleStatusChange(t,status);applyUpdatedTrip(updated||{...t,status});}catch(_){}}} onWorkflowAction={handleWorkflowAction} onAssignBus={setAssignTrip} onFinaliseTrip={setFinaliseTrip} onConfirmAction={(t,label,nextStatus,workflowAction)=>setConfirmAction({trip:t,label,nextStatus,workflowAction})} onView={setShowDetailsTrip} onEdit={setEditTrip} onSoftDelete={handleSoftDelete}/>{canSeePassengersButton(trip.status)&&<button onClick={()=>setShowPassengersTrip(trip)} className="flex items-center px-2 py-1 border rounded text-sm font-semibold text-gray-700"><RxPerson className="mr-1"/>{canMutatePassengers(trip)?"Manage Passengers":"View Passengers"}</button>}</div></td></tr>}</React.Fragment>)}
  ...
  {confirmAction&&<ConfirmActionPopup title={`${confirmAction.label} Trip`} description={`Are you sure you want to ${confirmAction.label.toLowerCase()} this trip?`} onConfirm={async()=>{const current=confirmAction;setConfirmAction(null);try{const updated=current.workflowAction?await handleWorkflowAction(current.trip,current.workflowAction):await handleStatusChange(current.trip,current.nextStatus);applyUpdatedTrip(updated);}catch(_){}closeRow();}} onClose={()=>setConfirmAction(null)}/>} 
