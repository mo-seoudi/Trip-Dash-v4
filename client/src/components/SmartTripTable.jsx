...
const SmartTripTable = ({ data, dateSortOrder, setDateSortOrder, refreshCallback }) => {
  ...
  const { paginatedData, currentPage, setCurrentPage, rowsPerPage, setRowsPerPage, jumpPageInput, setJumpPageInput, handleJump, totalPages } = usePagination(data);
  const { subTripsMap } = useSubTrips(data);
  const { showDetailsTrip, setShowDetailsTrip, confirmRejectTrip, setConfirmRejectTrip, handleStatusChange, handleViewTrip, handleCancel } = useTripActions(refreshCallback);
