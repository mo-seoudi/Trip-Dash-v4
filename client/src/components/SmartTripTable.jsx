...
const SmartTripTable = ({ trips, dateSortOrder, setDateSortOrder, readOnly = false }) => {
  ...
  const canSeePassengersButton=status=>canReadPassengers&&["Accepted","Confirmed","Completed"].includes(status);
  ...
  {assignTrip&&<ModalWrapper onClose={()=>setAssignTrip(null)}><AssignBusForm trip={assignTrip} onClose={()=>setAssignTrip(null)} onSubmit={updated=>{setTripData(prev=>prev.map(t=>t.id===updated.id?updated:t));window.dispatchEvent(new CustomEvent("trip:updated",{detail:updated}));setAssignTrip(null);closeRow();setShowDetailsTrip(updated);}}/></ModalWrapper>}
  ...
  {editTrip&&<ModalWrapper onClose={()=>setEditTrip(null)}><EditTripForm trip={editTrip} onClose={()=>setEditTrip(null)} onUpdated={updated=>{if(updated?.id){setTripData(prev=>prev.map(t=>t.id===updated.id?updated:t));window.dispatchEvent(new CustomEvent("trip:updated",{detail:updated}));setShowDetailsTrip(updated);}}} isRequestMode={profile?.role==="school_staff"&&editTrip?.status==="Confirmed"}/></ModalWrapper>}
 </div>;
