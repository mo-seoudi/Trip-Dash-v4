...
const normalizeTrip = (trip) => ({ ...trip, date: trip?.date ? new Date(trip.date) : null, returnDate: trip?.returnDate ? new Date(trip.returnDate) : null });
const sortTrips = (a,b) => { if(typeof a.id==="number"&&typeof b.id==="number")return b.id-a.id;if(a.createdAt&&b.createdAt)return new Date(b.createdAt)-new Date(a.createdAt);return 0; };
...
const unwrapList = (response) => { const value=unwrapData(response);return Array.isArray(value)?value:[]; };
...
export const requestWorkspaceQuotationApproval = async (schoolId,tripId,payload) => unwrapData(await api.post(`${workspaceQuotationPath(schoolId,tripId)}/request-approval`,payload));
export const approveWorkspaceQuotation = async (schoolId,tripId) => unwrapData(await api.post(`${workspaceQuotationPath(schoolId,tripId)}/approve`));
export const requestWorkspaceQuotationChanges = async (schoolId,tripId,reason) => unwrapData(await api.post(`${workspaceQuotationPath(schoolId,tripId)}/request-changes`,{reason}));
